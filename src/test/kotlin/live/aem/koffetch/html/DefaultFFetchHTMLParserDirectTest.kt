    ...
    fun testDefaultParserRobustness() {
        ...
        val edgeCaseInputs =
            listOf(
                ...
                "<" + "x".repeat(1_000_000) + ">",
                // Large tag
                (1..1000).joinToString("") { "<div>" } + (1..1000).joinToString("") { "</div>" },
                // Deep nesting
                "<!DOCTYPE html>" + "x".repeat(100_000),
                // Large content
                "<html" + (1..1000).joinToString("") { " attr$it='value$it'" } + ">",
                // Many attributes
                "<html><body>" + (1..10000).joinToString("") { "<p>Paragraph $it</p>" } + "</body></html>",
                // Many elements
            )
